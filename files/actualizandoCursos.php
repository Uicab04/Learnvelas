...
if (isset($_POST['nombreVideo']) && !empty($_POST['nombreVideo'])) {
    $video_url_nuevo = htmlentities($_POST['nombreVideo'], ENT_NOQUOTES, "UTF-8");

    // Eliminar el video anterior si se proporciona un nuevo video
    if ($video_url_actual && file_exists("../videos/" . $video_url_actual)) {
        // Mostrar información de depuración
        // echo "Intentando eliminar video anterior: ../videos/" . $video_url_actual . "<br>";

        // Intentar eliminar el video anterior
        if (unlink("../videos/" . $video_url_actual)) {
            // echo "El video anterior fue eliminado correctamente.<br>";
        } else {
            // echo "Error al intentar eliminar el video anterior.<br>";
        }
    } else {
        // echo "No se encontró el video anterior para eliminar.<br>";
    }
} else {
    $video_url_nuevo = $video_url_actual; // Mantener el nombre actual de video si no se proporciona uno nuevo
}
...
$sql->bindParam(':video_url', $video_url_nuevo, PDO::PARAM_STR);
